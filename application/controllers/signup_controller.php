...
class Signup_Controller extends CI_Controller {
    ...
    public function signup(){        
        ...
        $this->form_validation->set_rules(
            'txt_email','Employee email', 'trim|required|valid_email|is_unique[employee.email]'
        );
        $this->form_validation->set_rules('txt_username','Username', 'trim|required');
        $this->form_validation->set_rules('txt_password','Password', 'required');
        $this->form_validation->set_rules(
            'txt_confirm_password', 'Password Confirmation', 'trim|required|matches[txt_password]'
        );
        ...
        if($this->form_validation->run() == false){
            ...
            if($this->Employee_Model->insertEmployee($data)){
                //send confirm mail (current always true)
                if($this->Employee_Model->sendEmail($this->input->post('txt_email'))){
                    $this->session->set_flashdata(
                        'login_msg', 
                        '<div class="alert alert-success text-center">Successfully logged in</div>'
                    );
                    redirect('Task_Controller/index');
                }else{
                    $this->session->set_flashdata(
                        'msg', 
                        '<div class="alert alert-danger text-center">Failed!! Please try again.</div>'
                    );
                    $this->load->view('header');
                    $this->load->view('signup_view');
                    $this->load->view('footer');
    ...
    function confirmEmail($hashcode){
        if($this->Employee_Model->verifyEmail($hashcode)){
            $this->session->set_flashdata(
                'verify', 
                '<div class="alert alert-success text-center">Email address is confirmed. Please login to the system</div>'
            );
